...
# ------------------------------------------------------ #  
    ...
    async def on_member_join(self, member):
        ...
        captchaChannel = member #self.bot.get_channel(data["captchaChannel"])
        ...
        if data["captcha"] is True:
            ...
            try:
                ...
                if msg.content == password:
                    ...
                    await captchaChannel.send(embed = embed, delete_after = 5)
                    embed = discord.Embed(title = self.bot.translate.msg(member.guild.id, "onJoin", "YOU_HAVE_BEEN_KICKED").format(member.guild.name), description = self.bot.translate.msg(member.guild.id, "onJoin", "MEMBER_FAILED_THE_CAPTCHA_REASON").format(member.guild.name), color = 0xff0000)
                ...
                await captchaChannel.send(embed = embed, delete_after = 5)
                try:
                    embed = discord.Embed(title = self.bot.translate.msg(member.guild.id, "onJoin", "YOU_HAVE_BEEN_KICKED").format(member.guild.name), description = self.bot.translate.msg(member.guild.id, "onJoin", "USER_HAS_EXCEEDED_THE_RESPONSE_TIME_REASON").format(member.mention), color = 0xff0000)
                    await member.send(embed = embed)
                    await member.kick() # Kick the user
                except Exception as error:
